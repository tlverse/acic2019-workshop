...
```{r tmle3-ate-spec}
ate_spec <- tmle_ATE(treatment_level = "Nutrition + WSH",
...
options. For example, you can disable CV-TMLE as follows:
...
```{r tmle3-manual-fit}
suppressWarnings({
  tmle_fit_manual <- fit_tmle3(tmle_task, targeted_likelihood, tmle_params,
...
as the old one was targeted to the ATE.
...
suppressWarnings({
  tmle_fit_multiparam <- fit_tmle3(tmle_task, targeted_likelihood, all_params,
                                   targeted_likelihood$updater)
})
