...
0. Think about potentially good effect modifers for the WASH Benefits data. What should
   $V$ be and why? Do we want to minimize or maximize the outcome? Construct an approprite 
   `sl3` library for $A$, $Y$ and $B$.
   ...
   with weight-for-height Z-score as the outcome.
...
```{r load-washb-data, message=FALSE, warning=FALSE, cache=FALSE, eval=TRUE}
...
```{r washb-data-npsem-mopttx, message=FALSE, warning=FALSE, cache=FALSE, eval=TRUE}
...
```{r sl3_lrnrs-WASH-mopttx, eval=TRUE}
# Initialize few of the learners:
...
```{r spec_init_WASH_simple_q2, eval=TRUE}
## Question 2:
...
```{r spec_init_WASH_simple_q3, eval=TRUE}
## Question 3:
