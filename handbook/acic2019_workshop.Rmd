...
* [post-workshop survey](https://forms.gle/P8SbwJBeaQjZHRRF9)
...
7. Fit the tmle with the `tmle3` function. At a minimum, this function will
   ...
   (2) the data; (3) the list of nodes, which defined the variable roles in Step
...
mean outcome under an ITR is the **value of the ITR**. Even more, suppose one wishes
...
patient? For example, one might seek to improve retention in
...
might be used to. In this chapter, we examine multiple examples of optimal individualized
treatment regimes
...
* We can see that the estimate of $psi_0$ is $0.56$, and that the confidence interval covers
...
think the distribution of allocated treatment changed?
...
0. Think about potentially good effect modifers for the WASH Benefits data. What should
   $V$ be and why? Construct an approprite `sl3` library for $A$, $Y$ and $B$.
   Based on the picked $V$, estimate the mean under the ITR for
   the main randomized intervention used in the WASH Benefits trial
   with weight-for-height Z-score as the outcome we want to maximize.

1. Using the WASH benefits data, extract the optimal ITR for each individual.
...
* Consider simpler rules:

```{r spec_init_WASH_simple_q2, eval=FALSE}
## Question 2:
...
* The stochastic intervention generates a counterfactual random variable
  ...
  W, U_Y)$, where the counterfactual outcome $Y_{d(A,W)} \sim
  \mathcal{P}_0^{\delta}$.
...
* The four standard assumptions presented in \ref{intro} are necessary in order
  ...
  via the statistical functional.
