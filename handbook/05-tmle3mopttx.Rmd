...
1. Differentiate between dynamic and optimal dynamic treatment regimes from
...
_individualized treatment regime_ (ITR), and the (counterfactual) population
mean outcome under an ITR is the _value of the ITR_. Even more, suppose one
...
is referred to as an _optimal ITR_ or the _optimal individualized treatment_.
...
In this chapter, we examine a simple example of optimal individualized treatment
...
[`tmle3mopttx` R package](https://github.com/tlverse/tmle3mopttx), which
...
`tmle3mopttx` to estimate optimal individualized treatments and the
...
* Denote $A \in \mathcal{A}$ as categorical treatment, where $\mathcal{A}
  \equiv \{a_1, \cdots, a_{n_A} \}$ and $n_A = |\mathcal{A}|$, with $n_A$
...
* We also define $\bar{Q}_{Y,0}(A,W) \equiv E_0[Y \mid A,W]$.
...
* Consider dynamic treatment rules $V \rightarrow d(V) \in \{a_1, \cdots,
  a_{n_A} \} \times \{1\}$, for assigning treatment $A$ based on $V \in W$.
...
* Optimal ITR ideally assigns treatment to individuals falling in strata in
  which the stratum-specific average treatment effect, the _blip_ function, is
...
* We define the blip function as:
  $$\bar{Q}_0(V) \equiv E_0[Y_1-Y_0 \mid V] \equiv E_0[\bar{Q}_{Y,0}(1,W) -
    \bar{Q}_{Y,0}(0,W) \mid V],$$ or the average treatment effect within a
    stratum of $V$.
...
randomized trial, we are guaranteed a consistent estimate of $E[Y_a \mid V]$
even if we get $\bar{Q}_{Y,0}(A,W)$ wrong!
...
* Note that the mean under the true optimal rule is $\psi=0.578$ for this data
...
* We also need to specify the type of blip we will use in this estimation
  problem, and the list of learners used to estimate the blip function.

* In addition, we need to specify whether we want to maximize the mean outcome
  under the rule (`maximize=TRUE`).
...
* The first one considers a setting where the user might be interested in a grid
  ...
  of potential effect modifiers (_Simpler Rules_).
...
* The second extension concerns implementation of realistic optimal individual
  ...
  or global positivity restraints are not realistic to implement (_Realistic
  Interventions_).
...
* We define _pseudo-blips_ as vector valued entities where the output for a
...
The mean under the true optimal rule is $\psi=0.625$, which is the quantity we
aim to estimate.
...
1. Note that we need to estimate $g_0(A|W)$ for a categorical $A$- therefore we
...
2. In addition, since the corresponding blip will be vector valued, we will have
   a column for each additional level of treatment. As such, we need to create
   multivariate learners with the helper function `create_mv_learners` that
   takes a list of initialized learners as input.
...
* We will be using the WASH Benefits data, corresponding to the Effect of water
  ...
  development in rural Bangladesh trial.
...
* We aim to estimate the optimal ITR and the corresponding value under the
  optimal ITR for the main intervention in WASH Benefits data.
...
* Our outcome of interest is the weight-for-height Z-score which we seek to
...
* We pick a few potential effect modifiers, including mother's education,
  ...
  of the socio-economic status of individuals involved in the trial.
