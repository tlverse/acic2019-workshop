...
head(washb_data, 3) %>% kable(format = "markdown", digits = 3)
```
...
head(sl_preds)
sl_fit$print() %>% kable(format = "markdown", digits = 3)
```
## Extensions
...
sl_fancy_fit$print() %>% kable(format = "markdown", digits = 3)
```
...
```{r CVsl, eval=FALSE}
CVsl <- CV_lrnr_sl(sl_fit, washb_task, loss_squared_error)
CVsl %>% kable(format = "markdown", digits = 3)
```
...
washb_varimp %>% kable(format = "markdown", digits = 3)
```
...
# take a quick peek
head(chspred, 3) %>% kable(format = "markdown", digits = 3)
```
